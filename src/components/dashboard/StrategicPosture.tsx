    ...
    <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-4 shadow-lg h-full flex flex-col">
      ...
      <div className="space-y-3 font-sans">
        {THEATERS.map((theater) => (
          <div key={theater.name} className="flex items-center justify-between group border-b border-slate-800/30 pb-2 last:border-0">
            <div className="flex flex-col">
              <span className="text-[10px] text-white font-bold tracking-tight uppercase group-hover:text-blue-400 transition-colors">
                {theater.name}
              </span>
              <div className="flex items-center gap-3 mt-1">
                <div className="flex items-center gap-1 opacity-60">
                  <Plane className="w-3 h-3 text-slate-400" />
                  <span className="text-[9px] text-slate-300">{theater.air}</span>
                </div>
                <div className="flex items-center gap-1 opacity-60">
                  <Anchor className="w-3 h-3 text-slate-400" />
                  <span className="text-[9px] text-slate-300">{theater.sea}</span>
                </div>
              </div>
            </div>
            <div className="text-right">
              <span className={`text-[9px] font-black uppercase tracking-tighter ${theater.color}`}>
                {theater.status}
              </span>
            </div>
          </div>
